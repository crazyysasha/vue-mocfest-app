<template>	
	<div v-if="!payIsLoading">
		<div
			class="border border-white shadow-2xl bg-black text-white max-w-sm"
			v-if="!globalError"
		>
			...
			<div class="flex flex-col font-neutralFace p-4 sm:p-6">
				...
				<div v-if="!updateState.order">
					...
					>
						<template #default="{ title, subtitle }">
							...
							<div
								class="text-[13px] leading-5 tracking-normal font-thin font-montserrat text-center"
								v-html="subtitle"
							></div>
						</template>
					...
					<c-select
						...
						:disabled="true"
						option-key="id"
						v-if="dates.length > 1 && createState.event.tickets.length > 0"
					>
						<template #default="{ title }">
							<div class="underline underline-offset-2">Дата</div>
							<div class="text-[13px] tracking-normal font-thin font-montserrat">
								{{ title }}
							</div>
						</template>
						<template #option="{ option }">
					...
					<c-select
						...
						:disabled="true"
						option-key="id"
						v-if="times.length > 1 && createState.event.tickets.length > 0"
					>
						<template #default="{ title }">
							<div class="underline underline-offset-2">Начало события</div>
							<div class="text-[13px] tracking-normal font-thin font-montserrat">
								{{ title }}
							</div>
						</template>
					...
					<div class="flex flex-col" v-if="createState.event.tickets.length > 0">
						<div
							class="text-rose-900 w-full"
							v-for="error in createValidation.quantity.$errors"
							:key="error.$uid"
						>
							{{ error.$message }}
						</div>
				...
				<div v-else-if="updateState.order">
					...
					>
						<template #default="{ title }">
							<div class="underline underline-offset-2">Способ оплаты</div>
							<div class="text-[13px] tracking-normal font-thin font-montserrat">
								{{ title }}
							</div>
						</template>
					...
					<div class="py-3">
						<div class="flex justify-between mb-2">
							<span class="text-lg text-neutral-500 font-montserrat">Событие</span>
							<span class="text-sm">{{ updateState.order.event.title }}</span>
						</div>
						<div class="flex justify-between mb-2">
							<span class="text-lg text-neutral-500 font-montserrat">Кол-во</span>
							<span class="text-sm">{{ updateState.order.quantity }} шт.</span>
						</div>
						<div class="flex justify-between">
							<span class="text-lg text-neutral-500 font-montserrat">Итого</span>
							<span class="text-sm">{{ updateState.order.price }} сум.</span>
						</div>
					</div>
				</div>
			</div>
